...
# -*- coding: utf-8 -*-
"""
Created on Wed Sep 27 11:18:29 2023
...
hdr_image[..., 0] = cv2.normalize(radianceMap_r, None, alpha=0, beta=255, norm_type=cv2.NORM_MINMAX)
hdr_image[..., 1] = cv2.normalize(radianceMap_g, None, alpha=0, beta=255, norm_type=cv2.NORM_MINMAX)
hdr_image[..., 2] = cv2.normalize(radianceMap_b, None, alpha=0, beta=255, norm_type=cv2.NORM_MINMAX)


hdr_image2 = np.zeros(images[0].shape, dtype=np.float32)
hdr_image2[..., 0] = radianceMap_r
hdr_image2[..., 1] = radianceMap_g
hdr_image2[..., 2] = radianceMap_b

image = hdr_image.astype(np.uint8)
...
gamma = 1.75
image= cv2.pow(image/255., 1.0/gamma)
fig = plt.figure('TM')   
plt.imshow(image)
plt.show()
